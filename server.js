...
// すべての関数で room.lastActivity を更新するように変更
...
io.on('connection', (socket) => {
    ...
    socket.on('createRoom', (data) => {
        ...
        rooms[roomId] = { 
            ...
            dealer: { hand: [], score: 0 }, 
            timers: {},
            lastActivity: Date.now() // lastActivity を初期化
        };
    ...
    socket.on('joinRoom', (data) => {
        ...
        updateRoomActivity(roomId); // アクティビティを更新
        socket.emit('roomJoined', { roomId });
        updateGameState(roomId);
    });
    ...
    function determineWinner(roomId) {
        ...
        if (room.round >= MAX_ROUNDS) {
            ...
            setTimeout(() => {
                try {
                    if (!rooms[roomId]) return; // ルームが既にない場合は何もしない
